...
def main():
    ...
    with open("rev_log.txt", "w") as log_file:
        for name in dish_voc:
            ...
            if(rev_dish_count[index] != 0):
                log_file.write( name + " exist in " + str(rev_dish_count[index]) +" reviews, accumulate stars: " + str(rev_dish_stars[index]) + "\n" )
    ...
    with open("rest_log.txt", "w") as log_file:
        for name in dish_voc:
            index = dish_voc[name]
            if(rev_dish_count[index] != 0):
                log_file.write( name + " exist in " + str(rest_dish_count[index]) +" restaurants, accumulate stars: " + str(rest_dish_stars[index]) + "\n" )
